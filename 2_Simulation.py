    ...
    def display_pivot_table(self):
        ...
        try:
            df_pivoted = self.df.pivot_table(
                index=["timestamp", "Machine_Type"], columns="_field", values="value", aggfunc="first"
            )
...
with sim_tab:
    ...
    if not MATLAB_AVAILABLE:
        ...
        matlab_script_path_sim = st.text_input(
            "MATLAB Script Directory Path", DEFAULT_MATLAB_SCRIPT_PATH, key="sim_path_input"
        )
        ...
        if st.button("▶️ Run MATLAB Simulation & Upload Data", key="run_sim_button"):
            ...
            if condition_sim == 0 and selected_machines_sim:
            ...
            if run_simulation_flag:
                loader = MatlabDataLoader(
                    condition_sim, matlab_script_path_sim, INFLUX_URL, INFLUX_TOKEN_SIM,
...
with viz_tab:
    ...
    with viz_query_col1:
        measurements_options = ["Beckhoff_1", "Siemens_1", "Rough_6"]
    ...
    if 'viz_dataframe' in st.session_state and isinstance(st.session_state['viz_dataframe'], pd.DataFrame):
        ...
        elif 'viz_measurement' in st.session_state:
