    ...
    **Data Requirement:** Upload a CSV file with historical data.
    ...
    if data_available:
        ...
        if st.button(f"🚀 Generate Forecast for {horizon_label}", key="generate_forecast_button"):
            ...
            # --- Denormalization ---
            try:
                forecast_df_denormalized = pd.DataFrame(selected_forecast_norm, columns=TARGET_COLUMNS)
                for col in TARGET_COLUMNS:
                    if col in scalers:
                        ...
                        if range_val != 0:
                            forecast_df_denormalized[col] = forecast_df_denormalized[col] * range_val + min_val
                        else:
            ...
            try:
                ...
                forecast_timestamps = pd.date_range(start=plot_start_time, periods=horizon_steps, freq='T')
